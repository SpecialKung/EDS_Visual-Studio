...
void DLC_PrMgr(UBYTE Val){
    ...
    //}
	DLC_ulOffset = (ULONG)pos_PGto1MM((UDOUBLE)PGBS);	// GFC1test
    ...
    //}
	pr[LEV_LEN] = (UWORD)pos_PGto1MM((UDOUBLE)DLC_ulPgBrd);	// GFC1test
        ...
        for(i = 1; i <= 0x4B; i ++){
		    //DLC_ulPgLev[i] = (U32xU32divU32(DLC_ulPosLev[i], 65536, COF_ulPls2MMgain))<<2;	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 source
		    //DLC_ulPgLev[i] = DLC_ulPgLev[i]*10;	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 source
            //if(DLC_PDO_DLC_Mode == DLC_MODE_APS)
            //{
            //    DLC_ulPgLev[i] = DLC_ulPosLev[i] << 1;
            //}
            //else
            //{
            //    DLC_ulPgLev[i] = (ULONG)(((UDOUBLE)DLC_ulPosLev[i] * 2621440 + (UDOUBLE)(COF_ulPls2MMgain >> 1)) / (UDOUBLE)COF_ulPls2MMgain);	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 new
            //}
			DLC_ulPgLev[i] = (ULONG)pos_1MMtoPG((UDOUBLE)DLC_ulPosLev[i]);	// GFC1test
        }
    ...
    else
    {
		...
		DLC_ulPgDD1 = (ULONG)pos_1MMtoPG((UDOUBLE)DLC_ulPosDD1);	// GFC1test
    ...
    DLC_ubLevCur = pr[LEV_CUR];
		...
		// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong add 20220902 -------
		...
		if((DLC_btCANOff)||(pr[SOFC]==5)){ //經由實體線路給定RUN
			DLC_btUP = (mfi_status&0x01)?1:0;
		}
		else if(pr[SOFC]==4){ //經由CAN_BUS給定方向
		...
		*/
		...
		//單位:mm/sec
		DLC_uwVR1 = ((UDOUBLE)DLC_uwJR1*(UDOUBLE)DLC_uwTR1*(UDOUBLE)DLC_uwTR1)/(UDOUBLE)2000000;	//V1理想速度變化量
		DLC_uwVR2 = ((UDOUBLE)DLC_uwJR2*(UDOUBLE)DLC_uwTR2*(UDOUBLE)DLC_uwTR2)/(UDOUBLE)2000000;	//V2理想速度變化量
		DLC_uwVR3 = ((UDOUBLE)DLC_uwJR3*(UDOUBLE)DLC_uwTR3*(UDOUBLE)DLC_uwTR3)/(UDOUBLE)2000000;	//V3理想速度變化量
		DLC_uwVR4 = ((UDOUBLE)DLC_uwJR4*(UDOUBLE)DLC_uwTR4*(UDOUBLE)DLC_uwTR4)/(UDOUBLE)2000000;	//V4理想速度變化量
		...
		//根據DLC_ulPgCnt去換算出DLC_ulPosVal (mm), DLC_ubLevCur
		//mod pulse to mm
...
//		DLC_ulPosVal = DLC_ulPgCnt/ulTmp;
        ...
        {
            //DLC_ulPosVal = (ULONG)(((UDOUBLE)DLC_ulPgCnt * (UDOUBLE)COF_ulPls2MMgain + 1310720) / 2621440); // Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 new
            DLC_ulPosVal = (ULONG)pos_PGto1MM((UDOUBLE)DLC_ulPgCnt);	// GFC1test
        }
		...
		}else if(DLC_PDO_DLC_Mode == DLC_MODE_APS)   // GFC APS Jerry.Sk.Tseng 2022/12/23
        {
            ...
            if(DLC_PDO_TX_Data_Mode)
            {
                DLC_PDO_ID004_B4.ub |= 0x80;    // Set Byte4 bit 7 as 1
                ulTmp3.ul = ulDLC_PDO_RX_APS_Pu;	// GFC1test
                DLC_PDO_ID004_B5.ub = ulTmp3.ub.b2;     //APS Hi Byte
                DLC_PDO_ID004_B6.ub = ulTmp3.ub.b1;     //APS Med Byte
                DLC_PDO_ID004_B7.ub = ulTmp3.ub.b0;     //APS Lo Byte

                
            }else
            {
                DLC_PDO_ID004_B4.ub &= 0x7F;    // Set Byte4 bit 7 as 0
                DLC_PDO_ID004_B5.ub = DLC_ubLevTar;     //TF
                DLC_PDO_ID004_B6.ub = 0x00;     //0
                DLC_PDO_ID004_B7.ub = 0x00;     //0                
            }
       }      
...
void DLC_Algorithm(void){
	...
	if(pr[DelayCmp] != 0)	//04-40(開關延遲補償    sec)
	{
	...
	*/	//----------------------------------------------------------------------------------------------------------------
	...
	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 new -------------------------------------
	//#16386 optimization for over landing cause from leveling sensor calibration , James, 2021/04/13
	//04-40(開關延遲補償    sec)
	...
	if((DLC_btWelExc == 0)&&(DLC_ulCurSpd <= ulTmp))
	{
		...
		{  
			...
			if(DLC_ubDIR == DIR_UP)
			{
				if((DLC_btDZN == 1)&&(DLC_btDZNOld == 0))	//進入水平前100mm
				{
					...
					//DLC_ulPgCnt = DLC_ulPgLev[DLC_ubLevCur]-(DLC_ulPgBrd>>1)+ulDelayCmpPg;	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 source
					DLC_ulPgCnt = DLC_ulPgLev[DLC_ubLevCur]-((DLC_ulPgBrd+1)>>1)+ulDelayCmpPg;	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 new
				}
				else if((DLC_btDZN == 0)&&(DLC_btDZNOld == 1))	//離開水平後100mm // Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 add
				{
					if(DLC_ubLevCur == DLC_ubLevSta)	// Mitong new 只在啟動樓執行一次
					{
						//DLC_ulPgCnt = DLC_ulPgLev[DLC_ubLevCur]+(DLC_ulPgBrd>>1)+ulDelayCmpPg;	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 source
						DLC_ulPgCnt = DLC_ulPgLev[DLC_ubLevCur]+((DLC_ulPgBrd+1)>>1)+ulDelayCmpPg;	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 new
					}
			...
			else if(DLC_ubDIR == DIR_DN)
			{
				if((DLC_btDZN == 1)&&(DLC_btDZNOld == 0))	//進入水平前100mm
				{
                	// 將James加的鋼索延伸補償移到此處,下行水平前LD由Off變On時執行補償 // Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 --
                	//#16023 Rope compensation start
	                if(DLC_ubLevTar == DLC_ubLevCur){  //進入水平前185mm	
                        ...
                        //if(DLC_PDO_DLC_Mode == DLC_MODE_APS) // GFC APS Jerry.Sk.Tseng 2022/12/23
                        //{
                        //    DLC_ulPgRopeCmp = (DLC_ulPosRopeCmp << 1);
                        //}
                        //else
                        //{
						...
						{
							DLC_ulPgRopeCmp = (ULONG)pos_1MMtoPG((UDOUBLE)DLC_ulPosRopeCmp);	// GFC1test	//鋼索延伸補償的Pulse
						}
                    }
                	else{
                    	DLC_ulPgRopeCmp = 0;
					...
					//	DLC_ulPgCnt = DLC_ulPgLev[DLC_ubLevCur]+(DLC_ulPgBrd>>1)-ulDelayCmpPg;	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 souce
					DLC_ulPgCnt = DLC_ulPgLev[DLC_ubLevCur]+((DLC_ulPgBrd+1)>>1)-ulDelayCmpPg + DLC_ulPgRopeCmp;// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 new
				}
				else if((DLC_btDZN == 0)&&(DLC_btDZNOld == 1))	//離開水平後100mm	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 add
				{
					if(DLC_ubLevCur == DLC_ubLevSta)	// Mitong new 只在啟動樓執行一次
		...
		else
		{
			...
			if(DLC_ubDIR == DIR_UP)
			{
				if((DLC_btLU == 1)&&(DLC_btLUOld == 0))//1st landing Calibration //進入水平前185mm,LU Off變On
				{
					if((DLC_ulPosUD1 != 0)&&(DLC_btUD1 == 1) && (Driver_ID == IEDS_DRIVER))//#13761 position calibration on UDS1, James, 2020/03/03
					{
						/*   [Artemis Add Sensor819 Function/Special/2022/06/06]
					...
					//DLC_ulPgCnt = DLC_ulPgLev[DLC_ubLevCur]-(DLC_ulPgBrd>>1)-(DLC_ulPgSen>>1)+ulDelayCmpPg;	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 source
					DLC_ulPgCnt = DLC_ulPgLev[DLC_ubLevCur]-((DLC_ulPgBrd+DLC_ulPgSen+1)>>1)+ulDelayCmpPg;	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 new
				}
				...
				else if(DLC_ubLevCur == DLC_ubLevSta)	// Mitong new 只在啟動樓執行一次
				{
					if((DLC_btLU == 0)&&(DLC_btLUOld == 1))//leaving Calibration	//離開水平15mm,LU On變Off 
					{
						DLC_ulPgCnt = DLC_ulPgLev[DLC_ubLevCur] + ((DLC_ulPgBrd - DLC_ulPgSen + 1) >> 1) + ulDelayCmpPg;
					}
					else if((DLC_btLD == 0)&&(DLC_btLDOld == 1))//離開水平185mm,LD On變Off // Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 add
					{
						DLC_ulPgCnt = DLC_ulPgLev[DLC_ubLevCur] + ((DLC_ulPgBrd + DLC_ulPgSen + 1) >> 1) + ulDelayCmpPg;
					}
				}
			...
			{
				if((DLC_btLD == 1)&&(DLC_btLDOld == 0))//1st landing Calibration //進入水平前185mm,LD Off變On
				{                 
                    // 將James加的鋼索延伸補償移到此處,下行水平前LD由Off變On時執行補償 // Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 --
                    //#16023 Rope compensation start
                    if(DLC_ubLevTar == DLC_ubLevCur){  //進入水平前185mm
					...
					//DLC_ulPgCnt = DLC_ulPgLev[DLC_ubLevCur]+(DLC_ulPgBrd>>1)+(DLC_ulPgSen>>1)-ulDelayCmpPg ;	 // Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 source
					DLC_ulPgCnt = DLC_ulPgLev[DLC_ubLevCur]+((DLC_ulPgBrd+DLC_ulPgSen+1)>>1)-ulDelayCmpPg + DLC_ulPgRopeCmp;	 // Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 new
				}
				...
				else if(DLC_ubLevCur == DLC_ubLevSta){	//只在啟動樓執行一次// Mitong new
					if((DLC_btLD == 0)&&(DLC_btLDOld == 1))//leaving Calibration //離開水平15mm,LD On變Off 
					{
						//DLC_ulPgCnt = DLC_ulPgLev[DLC_ubLevCur]-(DLC_ulPgBrd>>1)+(DLC_ulPgSen>>1)-ulDelayCmpPg ;
						DLC_ulPgCnt = DLC_ulPgLev[DLC_ubLevCur] - ((DLC_ulPgBrd - DLC_ulPgSen + 1) >> 1) - ulDelayCmpPg;
					}
					else if((DLC_btLU == 0)&&(DLC_btLUOld == 1))	//離開水平185mm,LU On變Off // Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 add
					{
						DLC_ulPgCnt = DLC_ulPgLev[DLC_ubLevCur] - ((DLC_ulPgBrd + DLC_ulPgSen + 1) >> 1) - ulDelayCmpPg;
					}
				}
	...
	// decel switch pos correction, 若DS_LEN為0,不做校正功能
	...
	// Task 268638 直接停靠-多段速加減速及S曲線 Mitong 20220616 add ---------------------------------
	...
	// eps dir chg after GEN_OK, Henry, 20170621
		...
		if(DLC_btEPS){
			...
			if(DLC_btParkTooFar && (Driver_ID == IEDS_DRIVER)){	// Task 268638 直接停靠-多段速加減速及S曲線 Mitong 20220516 new ------------------------------------------------	
				//IEDS再平層時會觸發此條件,檢修模式REV FWD off,速度還不沒有到達零速			
				//執行停車時,如果速度低於0.02m/s,則先加速到0.02m/s後再開始減速
				if((DLC_ubDec0SMode == MODE_NULL) && (DLC_ulCurSpd < (pr[LEV_SPD]*100)) && (DLC_ulCurSpd > 0)){ // 0.0m/s < speed < pr[LEV_SPD]							
					...
					//減速到0速 , S3單位ms , Dec單位mm/s^2 , S4的單位用ms
					t_s3 = 1000;//單位ms , t=a/j=((pr[JOGDEC]*10) / J(800mm/s^3)) * 1000=pr[JOGDEC] * 12.5 = 80 * 12.5 = 1000ms
					a_dec= 800;	//單位mm/s^2
					t_s4 = 2000;//單位ms , t=a/j=((pr[JOGDEC]*10) / J(400mm/s^3)) * 1000=pr[JOGDEC] * 25 = 80 * 25 = 2000ms 
		...
		DLC_ubtra = 0x04;
		DLC_ubMode = MODE_NULL;
		if(DLC_btParkTooFar && (Driver_ID == IEDS_DRIVER) && (DLC_ubDIR != DIR_NULL)){	// Task 268638 直接停靠-多段速加減速及S曲線 Mitong 20220616 new -------------		
			//對應 IED-S 檢修停止	
			//減速到0速 , S3單位ms , Dec單位mm/s^2 , S4的單位用ms		
			//有S3=0.000s(J3=無限大) , Dec=pr[FSD] 
			a_dec = pr[FSD];	//單位mm/s^2 
			t_s3 = 0;			//單位ms	
			if(pr[FSD] < 1000){
				//S4=pr[FSD](J4=1m/s^3=1000mm/s^3) 
				t_s4 = pr[FSD];		//單位ms,t=a/j=(a(pr[FSD] * 1) / j(1000mm/s^3)) * 1000=pr[FSD]
			}
			else if((pr[FSD] >= 1000) && (pr[FSD] < 2000)){
				//S4=pr[FSD](J4=2m/s^3=2000mm/s^3) 
				t_s4 = pr[FSD]>>1;		//單位ms,t=a/j=(a(pr[FSD] * 1) / j(2000mm/s^3)) * 1000=pr[FSD]/2
			}
			else if((pr[FSD] >= 2000) && (pr[FSD] < 3000)){
				//S4=pr[FSD](J4=4m/s^3=4000mm/s^3) 
				t_s4 = pr[FSD]>>2;		//單位ms,t=a/j=(a(pr[FSD] * 1) / j(4000mm/s^3)) * 1000=pr[FSD]/4
			}
			else{
				t_s4 = 0;
			}
			...
			DLC_ulCurSpd = Spd_FSD(pr[FSD], 100);	// Task 268638 直接停靠-多段速加減速及S曲線 Mitong 20220516 source
		}
	}
...
//		DLC_ulCurSpd = Spd_Linear(pr[JOGF]*10, pr[JOGACC], pr[JOGDEC]);
		...
		if(DLC_btParkTooFar && (Driver_ID == IEDS_DRIVER)){
			DLC_ulCurSpd = Spd_Linear(pr[LEV_SPD]*10, 10, 100); //acc 0.10m/s2, decel 1.00m/s2	// Task 268638 直接停靠-多段速加減速及S曲線 Mitong 20220516 new
		}
		else{
			DLC_ulCurSpd = Spd_Linear(pr[LEV_SPD]*10, 20, 20); //decel 0.2m/s2	// Task 268638 直接停靠-多段速加減速及S曲線 Mitong 20220516 source
		}
	}
		...
		//DLC_ulCurSpd = Spd_Linear(pr[RSQ_SPD]*10, pr[JOGACC], pr[JOGDEC]);
		if(DLC_btParkTooFar && (Driver_ID == IEDS_DRIVER)){
			DLC_ulCurSpd = Spd_Linear(pr[RSQ_SPD]*10, 10, 100);	 //acc 0.10m/s2, decel 1.00m/s2	// Task 268638 直接停靠-多段速加減速及S曲線 Mitong 20220516 new
		}
		else{
			DLC_ulCurSpd = Spd_Linear(pr[RSQ_SPD]*10, 20, 20);	// Task 268638 直接停靠-多段速加減速及S曲線 Mitong 20220516 source
		}
	}
   ...
   if(pr[Sensor819] != 0)
   {
    ...
    if((DLC_PDO_DLC_Mode == DLC_MODE_APS) && DLC_btModNor)
    {
    	...
    	ud2 = (UDOUBLE)pr[Lift_SPD] * 100 * DLC_udEncCnt;
    	ulspeed = (ULONG)((ud1 + (ud2>>1)) / ud2);  // Mitong 四捨五入
    }
	else
	{
...
ULONG Spd_NOR(ULONG ulCurSpd){
	...
	UBYTE	   i, ubj;
	SWORD	swadjstVal;	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 add

	fdec5.ul = cal_time(pr[FMAX],0);

	//1mm unit in Spd_NOR
	//uwCurSpd = ulCurSpd / 10;           // Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 source
	uwCurSpd = (ulCurSpd+5) / 10;	    // Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 new
...
*/
    ...
    if(DLC_ubLevTar!=0){
		...
		if(DLC_btUP){	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 new
			if(DLC_ubLevTar <= 50){
			...
			swadjstVal += pr[DisDly] ;	// 彈簧延伸補償 , 設+表示要延後停車,設-表示要提前停車
			if(DLC_btPosLULDCal){		// 開啟二次校正
				//swadjstVal++ ;			// 目標位置延後1mm才能停平層
			}
		}
		else if(DLC_btDN){
            if(DLC_ubLevTar <= 50){
	...
	if(DLC_ubDIR == DIR_UP || DLC_ubDIR == DIR_DN){
		...
		if((DLC_ubMode==MODE_NULL) && (DLC_ubLevTar!=0) && (DLC_ubLevTar!=DLC_ubLevCur)){
			DLC_Init();	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 add
			DLC_ubSubtra = 0x02;
			DLC_ulDStop = 0;
			uwSpd = 0;
			DLC_ubMode = MODE_S1;
			DLC_btDisTar0mm = 0;	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 add
			// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 del ---------
			/*
			DLC_uwS1Tmr = 0;
			DLC_uwS2Tmr = 0;
			DLC_uwS3Tmr = 0;
		...
		{		
			...
			//ud2 = udJ1*ud1*ud1/2000000;	// Issue 277400 高速梯有拖尾速及平層不準的問題 source
			//uwSpd = (UWORD)ud2;			// Issue 277400 高速梯有拖尾速及平層不準的問題 source
			ud2 = (udJ1*ud1*ud1+100000)/200000;	// Issue 277400 高速梯有拖尾速及平層不準的問題 new
			DLC_ulSpd0p1mm = (ULONG)ud2;		// Issue 277400 高速梯有拖尾速及平層不準的問題 new
			
			//accel
			//ud1 = (UDOUBLE)DLC_uwJR1*(UDOUBLE)DLC_uwS1Tmr/1000;	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 source
			ud1 = ((UDOUBLE)DLC_uwJR1*(UDOUBLE)DLC_uwS1Tmr+500)/1000;	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 new
			DLC_uwATmp = (UWORD)ud1;
		}
		else if(DLC_ubMode == MODE_ACC)	//MODE_ACC = 2
		{
			//S2 area
			ulD2 = EqCal_D2(uwCurSpd);	//用目前的速度,開始走S2行程,走完S2行程後會走多少距離
			//V-const (EqCal_D2)
			DLC_uwVc = DLC_uwV2;	//以目前的速度開始計算,當S2結束後,速度會是多少,單位mm/s
			//V-const area
			//ud1 = (UDOUBLE)DLC_uwVc*(UDOUBLE)DLC_uwTc/1000;	//等速段需要走的距離	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 source
			ud1 = ((UDOUBLE)DLC_uwVc*(UDOUBLE)DLC_uwTc+500)/1000;	//等速段需要走的距離	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 new
			ulDcon = (ULONG)ud1;
			//ulDcon = ((double)DLC_uwVc/1000.0)*((double)DLC_uwTc/1000.0)*1000.0;
			//T3, Tdec, T4
			EqCal_T3_Tdec_T4(DLC_uwVc);
			//S3 area
			ulD3 = EqCal_D3(DLC_uwVc);	//用DLC_uwVc(S3開始的速度),開始走S3行程,走完S3行程後會走多少距離
			//dec area
			ulDdec = EqCal_Ddec();
			//S4 area
			ulD4 = EqCal_D4();
			...
			//mode change
			//DLC_uwVc>=DLC_uwRateSpd時,進入MODE_CON段,去消耗目標距離
			if((ulDisTar <= ulDisStp) || (DLC_uwVc >= DLC_uwRateSpd))
			{
				DLC_ubSubtra = 0x06;
				DLC_ubMode = MODE_S2;
			...
			if(DLC_uwVc < DLC_uwRateSpd)
			{
				...
				//ud2 = udV1+((udAa*ud1)/1000);		// Issue 277400 高速梯有拖尾速及平層不準的問題 source
				//uwSpd = (UWORD)ud2;				// Issue 277400 高速梯有拖尾速及平層不準的問題 source
				ud2 = (udV1*10)+((udAa*ud1+50)/100);// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong new
				DLC_ulSpd0p1mm = (ULONG)ud2;		// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong new			
			}
			else
			{
				//uwSpd = uwCurSpd;			// Issue 277400 高速梯有拖尾速及平層不準的問題 source
				DLC_ulSpd0p1mm = ulCurSpd;	// Issue 277400 高速梯有拖尾速及平層不準的問題 new
			}
		...
		else if(DLC_ubMode == MODE_S2)	//MODE_S2 = 3
		{
			...
			//ud1 = udVc-(udJ2*ud1*ud1/2000000);	// Issue 277400 高速梯有拖尾速及平層不準的問題 source
			//uwSpd = (UWORD)ud1;					// Issue 277400 高速梯有拖尾速及平層不準的問題 source
			ud1 = (udVc*10)-((udJ2*ud1*ud1+100000)/200000);	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong new
			DLC_ulSpd0p1mm = (ULONG)ud1;			// Issue 277400 高速梯有拖尾速及平層不準的問題 new

			//over-spd protection
			//if(uwSpd >= (10*pr[Lift_SPD])){		// Issue 277400 高速梯有拖尾速及平層不準的問題 source
			//	uwSpd = 10*pr[Lift_SPD];
			if(DLC_ulSpd0p1mm >= (100*pr[Lift_SPD])){	// Issue 277400 高速梯有拖尾速及平層不準的問題 new
				DLC_ulSpd0p1mm = (ULONG)100*pr[Lift_SPD];		// Issue 277400 高速梯有拖尾速及平層不準的問題 new
			...
			*/
			...
			//Speed Output
			//uwSpd = DLC_uwVc;				// Issue 277400 高速梯有拖尾速及平層不準的問題 source
			DLC_ulSpd0p1mm = (ULONG)DLC_uwVc*10;	// Issue 277400 高速梯有拖尾速及平層不準的問題 new
		}
			...
			//ud2 = udVc-(udJ3*ud1*ud1/2000000);// Issue 277400 高速梯有拖尾速及平層不準的問題 source
			//uwSpd = (UWORD)ud2;				// Issue 277400 高速梯有拖尾速及平層不準的問題 source
			ud2 = (udVc*10)-((udJ3*ud1*ud1+100000)/200000);	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong new
			DLC_ulSpd0p1mm = (ULONG)ud2;			// Issue 277400 高速梯有拖尾速及平層不準的問題 new
		}
			...
			if(uwCurSpd > DLC_uwV4){
				...
				//ud2 = (ud1*udAd)/1000;	// Issue 277400 高速梯有拖尾速及平層不準的問題 source		
				//ud3 = udVc-udV3-ud2;		// Issue 277400 高速梯有拖尾速及平層不準的問題 source
				//uwSpd = (UWORD)ud3;		// Issue 277400 高速梯有拖尾速及平層不準的問題 source
				ud2 = (ud1*udAd+50)/100;	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong new		
				ud3 = ((udVc-udV3)*10)-ud2;	// Issue 277400 高速梯有拖尾速及平層不準的問題 new
				DLC_ulSpd0p1mm = (ULONG)ud3;// Issue 277400 高速梯有拖尾速及平層不準的問題 new
			}
			else
				//uwSpd = uwCurSpd;			// Issue 277400 高速梯有拖尾速及平層不準的問題 source
				DLC_ulSpd0p1mm = ulCurSpd;	// Issue 277400 高速梯有拖尾速及平層不準的問題 new
...
//		}
			...
			DLC_ulDStop = 0;
			DLC_ubMode = MODE_DYN;
			// Issue 277400 高速梯有拖尾速及平層不準的問題 Mitong add ---------
			if(ulDisTar == 0){
				DLC_btDisTar0mm = 1;
			}
			// -----------------------------------------------------
			
			//if(DLC_uwPeriodCnt > 0){	// GFC1test source
			if((DLC_uwPeriodCnt > 0) || (DLC_ulDisTar_Old == ulDisTar)){	// GFC1test new 位置有改變才重新計算動態J
				if(DLC_uwPeriodCnt > 0)	// GFC1test
				{
					DLC_uwPeriodCnt --;
				}
				...
				//spd output
				udJ4 = (UDOUBLE)DLC_uwJD;						//DLC_uwJD為S4的動態J,單位mm/s3
				udT4 = (UDOUBLE)DLC_uwTD-(UDOUBLE)DLC_uwTDCnt;
				//ud1 = udJ4*udT4*udT4/2000000;	//速度	// Issue 277400 高速梯有拖尾速及平層不準的問題 Mitong source
				//uwSpd = (UWORD)ud1;					// Issue 277400 高速梯有拖尾速及平層不準的問題 Mitong source	
				ud1 = (udJ4*udT4*udT4+100000) / 200000;	// Issue 277400 高速梯有拖尾速及平層不準的問題 Mitong new 四捨五入
				DLC_ulSpd0p1mm = (ULONG)ud1;			// Issue 277400 高速梯有拖尾速及平層不準的問題 Mitong new
				DLC_ubSubtra = 0x20;	
			}
			else{		// GFC1test new 位置有改變才重新計算動態J
				DLC_ulDisTar_Old = ulDisTar;			// GFC1test
				DLC_uwPeriodCnt = 9;
				DLC_uwTDCnt = 0;
				if(!DLC_btDisTar0mm && (ulCurSpd > 50)){
					...
					*/
					...
					ud3 = ((UDOUBLE)8*ud1*ud1*ud1)/((UDOUBLE)36*ud2*ud2);	//用目前的速度及距離算出J值			
					DLC_uwJD = (UWORD)ud3;	//DLC_uwJD為S4的動態J,單位mm/s3
					*/
					// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 new -----------------------------
					...
					*/
					...
					//動態J限制					
					if(DLC_uwJD > uw1){
						if((DLC_uwJD - uw1) > 5000){	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 15改1000 // GFC1test
							DLC_uwJD = uw1 + 5000;
						}
					}

					if(DLC_uwJD > 50000){	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 add
						DLC_uwJD = 50000;
					}
					else
					{
				...
				//T=sqrt(2V/J), 開平方
				//ud1 = (UDOUBLE)2000000*(UDOUBLE)uwCurSpd/(UDOUBLE)DLC_uwJD;	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 source
				ud1 = ((UDOUBLE)200000*(UDOUBLE)ulCurSpd+(UDOUBLE)(DLC_uwJD>>1))/(UDOUBLE)DLC_uwJD;	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 source
				DLC_uwTD = uw_Sqrt32c_1((ULONG)ud1);
				
				//spd output
				udJ4 = (UDOUBLE)DLC_uwJD;
				udT4 = (UDOUBLE)DLC_uwTD;
				//ud1 = udJ4*udT4*udT4/2000000;		// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 source
				//uwSpd = (UWORD)ud1;				// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 source
				ud1 = (udJ4*udT4*udT4+100000)/200000;	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 new 四捨五入
				DLC_ulSpd0p1mm = (ULONG)ud1;		// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 new
				DLC_ubSubtra = 0x21;	
			}

			if(DLC_ulSpd0p1mm > ulCurSpd){	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 add
				DLC_ulSpd0p1mm = ulCurSpd;
			}
			if(DLC_ulSpd0p1mm < 10){		// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 add
				DLC_ubMode = MODE_DON;
				DLC_ubSubtra = 0x30;
			}
	        ...
	        //up over level
  			if(DLC_ubDIR == DIR_UP){
				//if(DLC_ulPosVal > uladjstVal){	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 source
				if((DLC_ulPosVal > uladjstVal) && ((DLC_ulPosVal - 10) > uladjstVal)){	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 new
					DLC_ubSubtra = 0x15;	// Sean, 20181207	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 del ROM不足
           			DLC_ubMode = MODE_DON;         
            		//uwSpd = 0;		// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 source
					DLC_ulSpd0p1mm = 0;	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 new
				}
			...
			DLC_ulSpd0p1mm = 0;	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 new
			if(fcmd.uw.hi==0){
				//DLC_ubMode = MODE_NULL;	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 del
				DLC_Init();
				DLC_ubSubtra = 0x23;
			}
		}
			...
			DLC_ubSubtra = 0x13;
			//uwSpd = 0;	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 del
			DLC_Init();		// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 add
		}	
	}
	else{
		DLC_ubSubtra = 0x14;
		//uwSpd = 0;				// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 del
		//ulDisStp = 0;				// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 del
		//DLC_ubMode = MODE_NULL;	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 del
		DLC_Init();	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 add
	}	
	
	if((DLC_ubMode == MODE_DON) || (DLC_ubMode == MODE_NULL)){	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 add
		DLC_ulSpd0p1mm = 0;
	}
	
	//ulSpd = uwSpd * 10;		// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 source
	//return ulSpd;			// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 source
	return DLC_ulSpd0p1mm;	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 new	
}

// Task 268638 直接停靠-多段速加減速及S曲線 Mitong 20220616 add
ULONG Spd_Dec0S(UWORD uwS3, UWORD uwDec, UWORD uwS4)	//有S3 S4減速到0速,S3 S3的單位用ms,Dec的單位用mm/s
{
// uwS3 uwS4單位0.00s
// uwDec 單位0.00m/s^2
	...
	uwSpd = uwCurSpd;
	if(DLC_ulCurSpd != 0){			
		if(DLC_ubDec0SMode == MODE_NULL){	//第一次進入Spd_Dec0S
			//DLC_ubSubtra = 0xe0;			
			DLC_uwVc = uwCurSpd;//單位:mm/s		
			DLC_uwAd = uwDec;	//單位:mm/sec^2
			
			//計算速度曲線
			//DLC速度基本參數讀取
			DLC_uwTR3 = uwS3;	//單位:ms
			DLC_uwTR3 = (DLC_uwTR3==0)?1:DLC_uwTR3;	//minimum protection
			DLC_uwJR3 = ((UDOUBLE)1000*(UDOUBLE)DLC_uwAd)/(UDOUBLE)DLC_uwTR3;	//單位:mm/sec^3
			DLC_uwVR3 = ((UDOUBLE)DLC_uwJR3*(UDOUBLE)DLC_uwTR3*(UDOUBLE)DLC_uwTR3)/(UDOUBLE)2000000;	//V3理想速度變化量	//單位:mm/sec

			DLC_uwTR4 = uwS4;	//單位:ms
			DLC_uwTR4 = (DLC_uwTR4==0)?1:DLC_uwTR4;	//minimum protection						
			DLC_uwJR4 = ((UDOUBLE)1000*(UDOUBLE)DLC_uwAd)/(UDOUBLE)DLC_uwTR4;	//單位:mm/sec^3			
			DLC_uwVR4 = ((UDOUBLE)DLC_uwJR4*(UDOUBLE)DLC_uwTR4*(UDOUBLE)DLC_uwTR4)/(UDOUBLE)2000000;	//V4理想速度變化量	//單位:mm/sec
			
			//T3, Tdec, T4
			EqCal_T3_Tdec_T4(uwCurSpd);
			DLC_uwT3_Dec0S = DLC_uwT3;	//運算後S3實際要走的時間,單位 ms
			DLC_uwV3_Dec0S = DLC_uwV3;	//從S3開始到S3結束,速度會減少多少,單位 mm/s
			DLC_uwJR3_Dec0S = DLC_uwJR3;//S3的J,,單位 mm/s^3

			DLC_uwTd_Dec0S = DLC_uwTd;	//等減速段要走的時間,單位 ms
			DLC_uwVd_Dec0S = DLC_uwVd;	//等減速段要走的速度變化量,單位 mm/s
			
			DLC_uwT4_Dec0S = DLC_uwT4;	//運算後S4實際要走的時間,單位 ms		
			DLC_uwV4_Dec0S = DLC_uwV4;	//S4開始時的速度,單位 mm/s
			DLC_uwJR4_Dec0S = DLC_uwJR4;//S4的J,,單位 mm/s^3
						
			DLC_uwVc_Dec0S = DLC_uwVc;	//減速開始時的速度,單位 mm/s
			DLC_uwAd_Dec0S = DLC_uwAd;	//減速度,單位 mm/s^2
		...
		if(DLC_ubDec0SMode == MODE_S3){	//MODE_S3 = 5
			if(uwS3 != 0){
				//Speed Output
				//uwSpd = DLC_uwVc-(DLC_uwJR3*DLC_uwS3Tmr*DLC_uwS3Tmr/2000000);
				DLC_uwS3Tmr = (DLC_uwS3Tmr < 65535)?(DLC_uwS3Tmr+1):65535;	//S3目前已運轉的時間,單位ms			
			...
			DLC_uwDecTmr = (DLC_uwDecTmr<65535)?(DLC_uwDecTmr+1):65535;	//等減速段目前已運轉的時間,單位ms
			if((uwS3 != 0) && (uwS4 != 0)){			
				//Speed Output
				if(uwCurSpd > DLC_uwV4_Dec0S){	//S4開始時的速度
...
//    uwtest25 = uwSpdFSD;
	...
	//if(DLC_ubCnt < 10){	// Task 268638 直接停靠-多段速加減速及S曲線 Mitong 20220516 source
    if(DLC_ubCnt < 9){	// Task 268638 直接停靠-多段速加減速及S曲線 Mitong 20220516 new
        DLC_ubCnt ++;
        ulSpd = ulCurSpd;
    }
    else{
...
ULONG EqCal_D2(UWORD uwCurSpd){
	...
	if(uwCurSpd >= DLC_uwVR2){
		...
		//計算T2
		//ud1 = (UDOUBLE)10000*2*udCurSpd/udJ2;	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 source
		ud1 = ((UDOUBLE)20000*udCurSpd+(udJ2>>1))/udJ2;	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 new
		L1 = (ULONG)ud1;
		DLC_uwT2 = 10*uw_Sqrt32c_1(L1);
		udT2 = (UDOUBLE)DLC_uwT2;
		
		//計算V2末速度
		//ud1 = (udJ2*udT2*udT2)/(UDOUBLE)2000000;	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 source
		ud1 = (udJ2*udT2*udT2+(UDOUBLE)1000000)/(UDOUBLE)2000000;	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 new
		DLC_uwV2 = uwCurSpd+(UWORD)ud1;	//以目前的速度開始計算,當S2結束後,速度會是多少,單位mm/s
		udV2 = (UDOUBLE)DLC_uwV2;
	}
			
	//ud1 = udJ2*udT2*udT2*udT2/6000000000;	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 source
	ud1 = (udJ2*udT2*udT2*udT2+3000000000)/6000000000;	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 new
	//ud2 = udV2*udT2/1000;		// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 source
	ud2 = (udV2*udT2+500)/1000;	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 new
	ud3 = ud2-ud1;
	L1 = (ULONG)ud3;
...
ULONG EqCal_D3(UWORD uwVcon){
	...
	//ud1 = udJ3*udT3*udT3*udT3/6000000000;	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 source
	ud1 = (udJ3*udT3*udT3*udT3+3000000000)/6000000000;	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 new
	//ud2 = udVc*udT3/1000;	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 source
	ud2 = (udVc*udT3+500)/1000;	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 new
	ud3 = ud2 - ud1;
	L1 = (ULONG)ud3;
...
ULONG EqCal_D4(void){
	...
	//ud1 = (udJ4*udT4*udT4*udT4)/6000000000;	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 source
	ud1 = (udJ4*udT4*udT4*udT4+3000000000)/6000000000;	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 new
	L1 = (ULONG)ud1;
...
ULONG EqCal_Ddec(void){
	...
	//ud1 = ((udV4+udV4+udVd)*udTd)/2000;	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 source
	ud1 = (((udV4+udV4+udVd)*udTd)+1000)/2000;	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 new
	L1 = (ULONG)ud1;
...
void EqCal_T3_Tdec_T4(UWORD uwVcon){
	...
	if((DLC_uwVR3 + DLC_uwVR4) <= uwVcon){
		...
		//Td = (Vc-V3-V4)/Ad
		//udTd = (UDOUBLE)1000*(udVc-udV3-udV4)/udAd;	//等減速段要走的時間 ms// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 source
		udTd = ((UDOUBLE)1000*(udVc-udV3-udV4)+(udAd>>1))/udAd;	//等減速段要走的時間 ms// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 new
		DLC_uwTd = (UWORD)udTd;	//等減速段要走的時間 ms
		
		//Vd = Ad*Td
		//udVd = udAd*udTd/1000;		//等減速段要走的時間 ms// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 source
		udVd = (udAd*udTd+500)/1000;	//等減速段要走的時間 ms// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 new
		DLC_uwVd = (UWORD)udVd;	//等減速段要走的速度變化量
	}
	else{
		//T3 = sqrt{(2*Vc)/[J3+(J3*J3/J4)]}		
		ud1 = 2*udVc;
		//ud2 = udJ3+((udJ3*udJ3)/udJ4);//等減速段要走的時間 ms// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 source
		ud2 = udJ3+((udJ3*udJ3+(udJ4>>1))/udJ4);//等減速段要走的時間 ms// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 new
		
		//ud3 = (UDOUBLE)(10000*ud1)/ud2;	//等減速段要走的時間 ms// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 source
		ud3 = (UDOUBLE)(10000*ud1+(ud2>>1))/ud2;	//等減速段要走的時間 ms// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 new
		L1 = (ULONG)ud3;
		DLC_uwT3 = 10*uw_Sqrt32c_1(L1);
		udT3 = (UDOUBLE)DLC_uwT3;
	
		//T4 = T3*J3/J4
		//udT4 = udT3*udJ3/udJ4;	//等減速段要走的時間 ms// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 source
		udT4 = (udT3*udJ3+(udJ4>>1))/udJ4;	//等減速段要走的時間 ms// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 new
		DLC_uwT4 = (UWORD)udT4;
		
		//Td = 0
		udTd = 0;
		DLC_uwTd = 0;			////等減速段, 減速時間
		
		//V3=J3*T3*T3/2
		//udV3 = udJ3*udT3*udT3/2000000;	//等減速段要走的時間 ms// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 source
		udV3 = (udJ3*udT3*udT3+1000000)/2000000;	//等減速段要走的時間 ms// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 new
		DLC_uwV3 = (UWORD)udV3;	//從S3開始到S3結束,速度會減少多少,單位mm/s
		
		//V4=J4*T4*T4/2
		//udV4 = udJ4*udT4*udT4/2000000;	//等減速段要走的時間 ms// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 source
		udV4 = (udJ4*udT4*udT4+1000000)/2000000;	//等減速段要走的時間 ms// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 new
		DLC_uwV4 = (UWORD)udV4;	//S4開始時的速度
...
void WelTunProc(void){
	...
	// DIR_UP and do the well-tune
		...
		//#16386 optimization for over landing cause from leveling sensor calibration , James, 2021/04/13
		//ulDelayCmpmm = DLC_ulCurSpd * (pr[DelayCmp] + pr[DIN_RES]);//pr[DIN_RES];//m/s(dot4)*sec(dot3)-->mm(dot7), 4+3=7	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 new
		//ulDelayCmpPg = U32xU32divU32(ulDelayCmpmm , 65536, COF_ulPls2MMgain)/250;//mm(dot7)*Q16/gain*Q2/1000 // Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 source		
		ulDelayCmpmm = ((DLC_ulCurSpd * (pr[DelayCmp] + pr[DIN_RES])) + 5000) / 10000;		
        //if(DLC_PDO_DLC_Mode == DLC_MODE_APS) // GFC APS Jerry.Sk.Tseng 2022/12/23
        //{
        //    udTmp = (250 + 1) >> 1;		
        //}
        //else
        //{
        //    udTmp = COF_ulPls2MMgain * 250;	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 new
        //}
		ulDelayCmpPg = (ULONG)pos_1MMtoPG((UDOUBLE)ulDelayCmpmm);	// GFC1test	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 new
			...
			if(DLC_btDznMd == 0){           // adco
				...
				{					
					if(DLC_btLU == 1 && DLC_btLUOld == 0){
						//因為紀錄1F位置的時候, DLC_ulPgSen尚未完成
						//因此, 在2F的位置時, 去修正1F位置
						if(DLC_ubLevCur == 2)
						{
							//DLC_ulPgLev[1] = DLC_ulPgLev[1]-(DLC_ulPgBrd>>1)+(DLC_ulPgSen>>1)-ulDelayCmpPg;	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 source
							DLC_ulPgLev[1] = DLC_ulPgLev[1] - ((DLC_ulPgBrd - DLC_ulPgSen + 1) >> 1) - ulDelayCmpPg;	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 new
						}
						//DLC_ulPgLev[DLC_ubLevCur] = DLC_ulPgCnt+(DLC_ulPgBrd>>1)+(DLC_ulPgSen>>1)-ulDelayCmpPg;	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 source
						DLC_ulPgLev[DLC_ubLevCur] = DLC_ulPgCnt + ((DLC_ulPgBrd + DLC_ulPgSen + 1) >> 1) - ulDelayCmpPg;	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 new
						DLC_uwWelTra = 0x07;
					}
					else
						DLC_uwWelTra = 0x08;
...
void WelTunProcAPS(void){
	...
	// DIR_UP and do the well-tune
		...
		if(DLC_ubDIR == DIR_UP){
			...
			if((DLC_btDD1Old == 1)&&(DLC_btDD1 == 0))
            {
                DLC_ulPgDD1 = ulDLC_PDO_RX_APS_Pu;
            }
            
			//DIP_UP, into UDS
			if((DLC_btUD1Old == 0)&&(DLC_btUD1 == 1))
            {
            	DLC_ulPgUD1 = ulDLC_PDO_RX_APS_Pu;
            }         

		}
	}
    ulDLC_PDO_RX_APS_ClipPuOld = ulDLC_PDO_RX_APS_ClipPu;
}
...
void WelTun_eeprom(void){
    ...
    if(DLC_PDO_DLC_Mode == DLC_MODE_APS)
    {
        // APS to mm
          for(i = 1; i <= 0x4B; i ++){
              ...
              DLC_ulPosLev[i] = (DLC_ulPgLev[i] + 1 ) >> 1;
          ...
          DLC_ulPosDD1 = (DLC_ulPgDD1 + 1 ) >> 1;
          DLC_ulPosDD2 = 0;
          DLC_ulPosDD3 = 0;
          DLC_ulPosDD4 = 0;

          
          DLC_ulPosUD1 = (DLC_ulPgUD1 + 1 ) >> 1;
          DLC_ulPosUD2 = 0;
          DLC_ulPosUD3 = 0;
          DLC_ulPosUD4 = 0;
          DLC_ulPosUL = 0;
    ...
    else
    {
        ...
        for(i = 1; i <= 0x4B; i ++){
            //單位轉換-PULSE to mm
            //DLC_ulPosLev[i] = U32xU32divU32((DLC_ulPgLev[i]>>2), COF_ulPls2MMgain, 65536);        // Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 source
            //DLC_ulPosLev[i] = DLC_ulPosLev[i]/10; // Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 source
            //DLC_ulPosLev[i] = (ULONG)(((UDOUBLE)DLC_ulPgLev[i] * (UDOUBLE)COF_ulPls2MMgain + 1310720) / 2621440);// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 new
            DLC_ulPosLev[i] = (ULONG)pos_PGto1MM((UDOUBLE)DLC_ulPgLev[i]);	// GFC1test
            j = i - 1;
            DLCxx[j<<1] = DLC_ulPosLev[i]/1000; //m
            DLCxx[((j<<1)+1)] = DLC_ulPosLev[i]%1000; //mm
	...
	//pr[LEV_LEN] = (U32xU32divU32((DLC_ulPgBrd>>2), COF_ulPls2MMgain, 65536))/10;// vrd length Aevin 7/6/2018	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 source
    ...
    //else
    //{
    //    pr[LEV_LEN] = (ULONG)(((UDOUBLE)DLC_ulPgBrd * (UDOUBLE)COF_ulPls2MMgain + 1310720) / 2621440); // Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 new
    //}
	pr[LEV_LEN] = (ULONG)pos_PGto1MM((UDOUBLE)DLC_ulPgBrd);	// GFC1test
	//Artemis speed limit, James, 20200220
...
void Update_C40xx(void){
    ...
    //ultmp = U32xU32divU32((PGBS>>2), COF_ulPls2MMgain, 65536);	// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 source
	//ultmp = ultmp/10;											// Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 source
    ...
    //else
    //{
    //    ultmp = (ULONG)(((UDOUBLE)PGBS * (UDOUBLE)COF_ulPls2MMgain + 1310720) / 2621440); // Issue 277400 高速梯有拖尾速及平層不準的問題 // Mitong 20220902 new
    //}
	ultmp = (ULONG)pos_PGto1MM((UDOUBLE)PGBS);	// GFC1test
...
//----------------------------------------------------------------------------------------
ulTmp = (SpDt_ulPG1Npulse>>2) * pr[FMAX];
COF_ulPls2MMgain = U32xU32divU32((pr[Lift_SPD]*COF_ubMPolePair), 655360000, ulTmp);

COF_ulPls2MMgain = (pr[Lift_SPD] * COF_ubMPolePair * 655360000) / ulTmp;
COF_ulPls2MMgain = (pr[Lift_SPD] * COF_ubMPolePair * 655360000 * 4) / (SpDt_ulPG1Npulse * pr[FMAX]);
//------------------------------------------------------------------------------------------
DLC_ulOffset = U32xU32divU32((PGBS>>2), COF_ulPls2MMgain, 65536);
DLC_ulOffset = DLC_ulOffset/10;

DLC_ulOffset = (PGBS * COF_ulPls2MMgain) / (4 * 65536 * 10);
DLC_ulOffset = (PGBS * pr[Lift_SPD] * COF_ubMPolePair * 655360000 * 4) / (4 * 65536 * 10 * SpDt_ulPG1Npulse * pr[FMAX]);
DLC_ulOffset = (PGBS * pr[Lift_SPD] * COF_ubMPolePair * 1000) / (SpDt_ulPG1Npulse * pr[FMAX]);
DLC_ulOffset = (PGBS * pr[Lift_SPD] * COF_uwPole * 500) / (SpDt_ulPG1Npulse * pr[FMAX]);
//-----------------------------------------------------------------------------------------
PGBS = (DLC_ulOffset * SpDt_ulPG1Npulse * pr[FMAX]) / (pr[Lift_SPD] * COF_uwPole * 500);
*/

// Function : PG_Val transfer to 0.1mm
UDOUBLE pos_PGto0d1MM(UDOUBLE pg_val)	// GFC1test
{
	...
	{	
		udtmp = (UDOUBLE)SpDt_ulPG1Npulse * (UDOUBLE)pr[FMAX];
		if(udtmp == 0)
		{	
			len = 0 ; 
		}
		...
		{	
			len = ((pg_val * (UDOUBLE)pr[Lift_SPD] * (UDOUBLE)COF_uwPole * 5000) + (udtmp >> 1)) / udtmp;
		}
	}	
	return len;
}

// Function : PG_Val transfer to 1mm
UDOUBLE pos_PGto1MM(UDOUBLE pg_val)	// GFC1test
{
	...
	{	
		...
		{		
			len = ((pg_val * (UDOUBLE)pr[Lift_SPD] * (UDOUBLE)COF_uwPole * 500) + (udtmp >> 1)) / udtmp;
		}
	}	
	return len;
}
...
// Function : 0.1mm transfer to PG_Val
UDOUBLE pos_0d1MMtoPG(UDOUBLE len)	// GFC1test
{
	...
	{
		udtmp = (UDOUBLE)pr[Lift_SPD] * 5000 * (UDOUBLE)COF_uwPole;
		if(udtmp == 0)
		{	
			pg_val = 0 ;
		}
		...
		{	
			pg_val = ((len * (UDOUBLE)SpDt_ulPG1Npulse * (UDOUBLE)pr[FMAX]) + (udtmp >> 1)) / udtmp;
		}
	}
	return pg_val;
}
...
UDOUBLE pos_1MMtoPG(UDOUBLE len)	// GFC1test
{
	...
	else
	{
		...
		{	
			udtmp = (UDOUBLE)pr[Lift_SPD] * 500 * (UDOUBLE)COF_uwPole;
			pg_val = ((len * (UDOUBLE)SpDt_ulPG1Npulse * (UDOUBLE)pr[FMAX]) + (udtmp >> 1)) / udtmp;
		}
	}
	return pg_val;
